...
let propagate [m][d][p][nr][nc][k]
  ...
  let knns' =
    loop (knns) for im1 < nr-1 do
      ...
      let (n_leavess, to_search_leavess) =
        opaque <| unzip <|
...
entry propagateFixK [m][d][p][n]
  ...
  let queries1 = queries0 :> [nr64*nc][d]f32
  let queries = unflatten queries1 :> [nr64][nc][d]f32

  let nat_leaves1 = nat_leaves0 :> [nr64*nc]i32
  let nat_leaves = unflatten nat_leaves1 :> [nr64][nc]i32

  let knns0 = map2 zip knn_inds knn_dsts
  ...
  let knn_inds'' = map (\kinds -> map (\ind -> if ind >= 0 then indir[ind] else -1i32) kinds) knn_inds'
