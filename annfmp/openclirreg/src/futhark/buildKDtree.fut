...
local let countMaxInDim [m] [d] (input: [m][d]f32) (indir: [m]i32)
                             (off: i64) (len: i64) (dim: i32)
                             : (i64, f32) =
    if len <= 0i64 || dim < 0i32 then
      (0i64, f32.lowest)
    else
      let (_, max_val) =
        loop (j, mx) = (0i64, f32.lowest)
        while j < len do
          let ind = indir[off+j]
          let x = input[ind, dim]
          in  (j+1i64, f32.max mx x)
      ...
      let (_, count) =
        ...
        while j < len do
          let ind = indir[off+j]
          let x = input[ind, dim]
          in  (j+1i64, if x == max_val then c+1i64 else c)

      in  (count, max_val)

-- Choose the first non-degenerate split dimension among the five largest-spread
-- dimensions.  A candidate is rejected when at least half of the points in the
-- node are equal to the maximum value in that candidate dimension.  If all five
-- candidates are rejected, fall back to the largest-spread dimension; the
-- rank-k-search patch can then move the split value from max to second max.
local let chooseSplitDimTop5 [m] [d] (input: [m][d]f32) (indir: [m]i32)
                                  (off: i64) (len: i64) (diffs: [d]f32)
                                  : i32 =
    let (best_dim, _) =
      reduce_comm (\ (i1,v1) (i2,v2) ->
                     if v1 >= v2 then (i1, v1) else (i2, v2))
                  (-1i32, f32.lowest) <|
      zip (map (\x -> i32.i64 x) (iota d)) diffs

    let selected0 = replicate 5 (-1i32)
    let (_, chosen, _) =
      loop (t, chosen, selected) = (0i32, -1i32, selected0)
      while t < 5i32 && chosen == -1i32 do
        let (cand_dim, _) =
          reduce_comm (\ (i1,v1) (i2,v2) ->
                         if v1 >= v2 then (i1, v1) else (i2, v2))
                      (-1i32, f32.lowest) <|
          map2 (\dim diff ->
                  let already_selected =
                    reduce_comm (||) false (map (== dim) selected)
                  in  if already_selected
                      then (-1i32, f32.lowest)
                      else (dim, diff))
               (map (\x -> i32.i64 x) (iota d))
               diffs

        let (num_max, _) = countMaxInDim input indir off len cand_dim

        -- Strictly less than 50%.  Equal to 50% is rejected.
        let candidate_ok = cand_dim >= 0i32 && num_max * 2i64 < len
        let chosen = if candidate_ok then cand_dim else chosen
        let selected = if cand_dim >= 0i32
                       then selected with [t] = cand_dim
                       else selected
        in  (t+1i32, chosen, selected)

    in  if chosen != -1i32 then chosen else best_dim
...
let mkKDtree [m] [d] (height: i32) (q: i64)
    ...
    loop ( indir  : *[m]i32
        ...
        for lev < (height+1) do
            let nodes_this_lvl = 1i64 << i64.i32 lev
            let cur_shp = shp[0:nodes_this_lvl]
            let cur_offsets = exScan (+) 0 cur_shp
            let (med_dims, anc_same_med) =
                map (\ (i: i32) ->
                        let node_ind = i + i32.i64 nodes_this_lvl - 1
                        -- walk from root to node and update bounds
                        ...
                        --     then copy lubs
                        --     else recomputeBoundsFromPoints input indir off len
                        -- choose the first usable dimension among the five highest-spread dimensions
                        let diffs = map (\i -> f32.abs(lubs_cur[i+d] - lubs_cur[i])) (iota d)
                        let _ = trace diffs
                        let off = cur_offsets[i]
                        let len = cur_shp[i]
                        let cur_dim = chooseSplitDimTop5 input indir off len diffs
                        let _ = trace cur_dim
                        let prev_anc = findClosestMed cur_dim median_dims node_ind
                        in  (cur_dim, prev_anc)
                    ) (map (\x -> i32.i64 x) (iota nodes_this_lvl))
