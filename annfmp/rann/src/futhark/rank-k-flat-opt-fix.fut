...
--type uiit = u16
--def i32_uiit = i32.u16
--def uiit_i64 = u16.i64
--def uiit_zero= 0u16

type uiit = u32
def i32_uiit = i32.u32
def uiit_i64 = u32.i64
def uiit_zero= 0u32
...
let rankSearchBatch [m][n] (meds: [m]f32) (ks: [m]i32)
                           (shp: [m]i32) (II1_o: *[n]uiit) (A_o: *[n]f32) : [m]f32 =
  let II1_bak = #[scratch] replicate n uiit_zero
  let A_bak = #[scratch] replicate n 0f32
  let res = replicate m 0f32
  let q = 0i64
  let nn = n
  --
  let (_, _, _shp, _,_,_,_,_, res) =
    loop (nn, ks:[m]i32, shp:[m]i32, II1_o:*[n]uiit, II1_bak:*[n]uiit, A_o:*[n]f32, A_bak:*[n]f32, q, res)
      while (nn > 0) do
        --
        let   A =   A_o[:nn]
        let II1 = II1_o[:nn]
        ...
        let h_inds =
            imap2 II1 A
                  (\ sgmindp1 a ->
                    let sgmind = i32_uiit sgmindp1 - 1
        ...
        let conds = opaque <| map2 keepElem II1 A
        -- ^ strange fusion with duplicating computation without opaque
        ...
        in  (tot_len, ks', shp', II1', II1_o, A', A_o, q+1, res')
  in res

-- ==
-- entry: main test
-- random input { [10][10000000]f32 }
-- random input { [100][1000000]f32 }
-- random input { [1000][100000]f32 }
-- random input { [10000][10000]f32 }

-- random input { [100000][1000]f32 }
...
--    let means = map (reduce_comm (+) 0f32) ass |> map (/ (f32.i64 n))
    ...
    let II1 = (tabulate_2d m n (\i _j -> uiit_i64 i + 1) |> flatten) :> *[N]uiit
